    ...
    def forward_train(self,
        ...
        if self.enable_vb:
            ...
            for meta in target_img_metas:
                if 'night' in meta['filename']:
                    ...
                    night_map.append(0)
            target_ib_img = night_fog_filter(target_img, means, stds, night_map, mode='hsv-s-w4')
            # target_img = night_fog_filter(target_img, means, stds, night_map, mode='hsv-s-w4')
        ...
        if not self.source_only:
            ...
            for i in range(batch_size):
                ...
                if self.enable_vb:
                    mixed_ib_img[i], mixed_seg_weight[i] = strong_transform(
                        strong_parameters,
                        data=torch.stack((img[i], target_ib_img[i])),
            ...
            mix_loss.backward()
            # del mix_loss,mix_losses
            
            if self.enable_vb:
                # Train on mixed ib images
                ...
                seg_debug['Mix_ib'] = self.get_model().debug_output
                mix_ib_losses = add_prefix(mix_ib_losses, 'mix_ib')
                mix_ib_loss, mix_ib_log_vars = self._parse_losses(mix_ib_losses)
                log_vars.update(mix_ib_log_vars)
